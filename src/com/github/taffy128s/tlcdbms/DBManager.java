...
public class DBManager implements DiskWritable {
    ...
    private boolean setSortingTarget(SQLParseResult parameter) {
        ...
        if (tablename == null) {
            int found = -1;
            for (String table : parameter.getTablenames()) {
                int index = mTables.get(table).getAttributeNames().indexOf(attribute);
                if (index != -1 && found != -1) {
            ...
            if (mTables.get(tablename).getAttributeNames().indexOf(attribute) == -1) {
